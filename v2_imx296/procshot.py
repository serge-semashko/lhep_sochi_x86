...
def process_shot(kx, ky, xl, yu, xr, yd, xdlin, ydlin, nx, ny, filename, thick_center, thick_cm, thick_5mm, mark=1):
    ...
    rezimg1 = np.zeros([yrange, xrange, 3], dtype=np.uint8)
        ...
        for j in range(rezmatr1.shape[0]):
            ...
            for a in range(yu1, yd1):
                for b in range(xl1, xr1):
                    rezimg1[a][b] = sr1
    print(rezimg1.shape)
    print('vert %d %d'%(center_x, yu))
    print('vert %d %d'%(center_x, yd))
    print('hor %d %d'%(xl, center_y))
    print('hor %d %d'%(xr, center_y))
    cv2.imwrite('test-1cm.png', rezimg1)
    cv2.line(rezimg1, (1, 1), (10, 10), (0, 0, 0), thickness=thick_center)
    ...
    rezimg2 = np.zeros([yrange, xrange, 3], dtype=np.uint8)
    print(rezimg2.shape)
    for i in range(rezmatr2.shape[1]):
        for j in range(rezmatr2.shape[0]):
    ...
    f_all.close()
    rezimg3 = np.zeros([yrange, xrange, 3], dtype=np.uint8)
