...
import ArducamSDK
tm = time.gmtime()
file_name = 'log_'+str(tm.tm_year) + '-' + str(tm.tm_mon) + '-' + str(tm.tm_mday) + '_' + str(tm.tm_hour) + '#' + str(tm.tm_min) + '#'+ str(tm.tm_sec)+'.txt'

log = open(file_name,'w',buffering=100)
print('test',file=log)
log.write('testwr')
...
def captureImage_thread():
    global handle,running,err_time, frames_noerr
    err_count = 0
    ...
    while running:
        ...
        if rtn_val > 255:
            print(err_count, " Error capture image, rtn_val = ",rtn_val)
            logging.info(str(err_count)+ " Error capture image, rtn_val = %d"%(rtn_val))
            if rtn_val == ArducamSDK.USB_CAMERA_USB_TASK_ERROR:
                break
            err_count+=1
            if err_count>10:    
                err_count = 0
                print('restart',file=log)
                logging.info('restart. time wo err=' +str(time.time()-err_time) +' frame wo err=' +str(frames_noerr))
                err_time=time.time()
                count=0
                frames_noerr=0
                time0=time.time()
                ...
                if rtn_val != 0:
                    print("Error beginning capture, rtn_val = ",rtn_val)
                    logging.info("Error beginning capture, rtn_val = %d"%(rtn_val))
                    running = False
                    return
                else:
                    print("Capture began, rtn_val = ",rtn_val)
                    print("Capture began, rtn_val = ",rtn_val,file=log)
            log.flush()
...
def readImage_thread():
    global handle,running,Width,Height,save_flag,cfg,color_mode,save_raw, frames_noerr, count, time0
    ...
    while running:
        ...
        if ArducamSDK.Py_ArduCam_availableImage(handle) > 0:		
            ...
            frames_noerr +=1
            if time1 - time0 >= 1:
                time_str = str(tm.tm_hour) + ':' + str(tm.tm_min) + '"'+ str(tm.tm_sec)+' '

                print("%s %s %d %s\n"%(time_str,"fps:",count,"/s"))
                print("%s %d %s\n"%("fps:",count,"/s"),file=log)
                log.flush()
                count = 0
                time0 = time1
            count += 1
