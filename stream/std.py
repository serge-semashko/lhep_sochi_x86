...
def camera_initFromFile(fileName):
    ...
    logging.info("color mode",color_mode)
    ...
    if ret == 0:
        ...
        logging.info("Serial: %c%c%c%c-%c%c%c%c-%c%c%c%c"%(datas[0],datas[1],datas[2],datas[3],
        ...
        logging.info("open fail,rtn_val = ",ret)
        return False
pass
...
def captureImage_thread():
    ...
    if rtn_val != 0:
        ...
        logging.info("Error beginning capture, rtn_val = ",rtn_val)
        running = False
        return
    else:
    ...
    while running:
        ...
        if rtn_val > 255:
            ...
            logging.info(err_count, "Error capture image, rtn_val = ",rtn_val)
            ...
            if err_count>10:    
                ...
                if rtn_val != 0:
                    print("Error beginning capture, rtn_val = ",rtn_val)
                    print("Error beginning capture, rtn_val = ",rtn_val,file=log)
                    running = False
                    return
                else:
                    print("Capture began, rtn_val = ",rtn_val)
...
def readImage_thread():
    ...
    global COLOR_BayerGB2BGR,COLOR_BayerRG2BGR,COLOR_BayerGR2BGR,COLOR_BayerBG2BGR, count, time0
