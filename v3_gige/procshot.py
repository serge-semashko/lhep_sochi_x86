...
def get_hsv_color(h):
    tresholds = [[0, 0, 255], [0, 255, 255], [0, 255, 0], [255, 255, 0], [255, 0, 0], [255, 0, 255], [0, 0, 255]]

    h = abs(h - 255)
    treshold = int(h / 60)
    steps = h % 60  # сколько шагов надо сделать по 4.25
    pix = tresholds[treshold].copy()

    pix[treshold % 3] += (-1) ** (treshold % 2) * 4.25 * steps
    if pix[treshold % 3] - int(pix[treshold % 3]) == 0.75:
        pix[treshold % 3] += 0.25

    return pix

def change_pic(imname):
    ...
    try:
        ...
        for i in range(len(pix_arr)):
            ...
            coords = pix_arr[i]
            if coords[0] > 0:
                sb += img[coords[0] - 1][coords[1]][0]
                sg += img[coords[0] - 1][coords[1]][1]
                sr += img[coords[0] - 1][coords[1]][2]
                k += 1
                if coords[1] > 0:
                    sb += img[coords[0] - 1][coords[1] - 1][0]
                ...
                if coords[1] < width:
                    ...
                    k +=1
            if coords[1] > 0:
                sb += img[coords[0]][coords[1] - 1][0]
                sg += img[coords[0]][coords[1] - 1][1]
                sr += img[coords[0]][coords[1] - 1][2]
                k += 1
            if coords[1] < width:
                sb += img[coords[0]][coords[1] + 1][0]
                sg += img[coords[0]][coords[1] + 1][1]
...
def process_shot(kx, ky, xl, yu, xr, yd, xdlin, ydlin, nx, ny, filename, thick_center, thick_cm, thick_5mm, mark):
    ...
    for i in range(rezmatr0.shape[1]):
        for j in range(rezmatr0.shape[0]):
            ...
            sr0 = get_hsv_color(rezmatr0[j][i][1] / 3)
            sr0_1 = get_hsv_color(get_changed_color(rmin0, rmax0, rezmatr0[j][i][1]))
            print(sr0)
    ...
    rezimg1_1 = np.zeros([yrange, xrange, 3], dtype=int)
    ...
    for i in range(rezmatr1.shape[1]):
        for j in range(rezmatr1.shape[0]):
            ...
            sr1 = get_hsv_color(rezmatr1[j][i][1] / 3)
            sr1_1 = get_hsv_color(get_changed_color(rmin1, rmax1, rezmatr1[j][i][1]))
    ...
    cv2.line(rezimg1_1, (center_x, yu), (center_x, yd), (0, 0, 0), thickness=thick_center)
    cv2.line(rezimg1_1, (xl, center_y), (xr, center_y), (0, 0, 0), thickness=thick_center)
    ...
    if mark == 1:
        ...
        for i in range(center_x, xl, -1 * range_cm):
            cv2.line(rezimg1_1, (i, yu), (i, yd), (0, 0, 0), thickness=thick_5mm)
        for i in range(center_x, xr, range_cm):
            cv2.line(rezimg1_1, (i, yu), (i, yd), (0, 0, 0), thickness=thick_5mm)
    ...
    rezimg2_1 = np.zeros([yrange, xrange, 3], dtype=int)

    print(rezimg2.shape)
    for i in range(rezmatr2.shape[1]):
        for j in range(rezmatr2.shape[0]):
            #sr2 = get_color(rezmatr2[j][i][1])
            sr2 = get_hsv_color(rezmatr2[j][i][1] / 3)
            sr2_1 = get_hsv_color(get_changed_color(rmin2, rmax2, rezmatr2[j][i][1]))
            # print(sr2)
    ...
    if mark == 1:
        ...
        for i in range(center_x, xr, range_mm):
            cv2.line(rezimg2, (i, yu), (i, yd), (0, 0, 0), thickness=thick_5mm)
        for i in range(center_x, xl, -1 * range_mm):
            cv2.line(rezimg2_1, (i, yu), (i, yd), (0, 0, 0), thickness=thick_5mm)
        for i in range(center_x, xr, range_mm):
            cv2.line(rezimg2_1, (i, yu), (i, yd), (0, 0, 0), thickness=thick_5mm)
    ...
    rezimg3_1 = np.zeros([yrange, xrange, 3], dtype=int)

    print(rezimg3.shape)
    for i in range(rezmatr3.shape[1]):
        for j in range(rezmatr3.shape[0]):
            #sr3 = get_color(rezmatr3[j][i][1])
            sr3 = get_hsv_color(rezmatr3[j][i][1] / 3)
            sr3_1 = get_hsv_color(get_changed_color(rmin3, rmax3, rezmatr3[j][i][1]))
            # print(sr3)
    ...
    if mark == 1:
        ...
        for i in range(center_x, xr, pmm_y):
            cv2.line(rezimg3, (i, yu), (i, yd), (0, 0, 0), thickness=thick_5mm)
        for i in range(center_x, xl, -1 * pmm_y):
            cv2.line(rezimg3_1, (i, yu), (i, yd), (0, 0, 0), thickness=thick_5mm)
        for i in range(center_x, xr, pmm_y):
            cv2.line(rezimg3_1, (i, yu), (i, yd), (0, 0, 0), thickness=thick_5mm)
...
if __name__ == "__main__":
    ...
    t1 = time.time()
    process_shot(1 ,  1,538, 358, 800, 600, 80, 57, 8, 6, 'images/gig00.png', 4, 1, 1,1)
#   process_shot(kx, ky, xl, yu, xr, yd, x_len, y_len, x_tab, y_tab, imname, w1, w2, w3, mark))
